...
class PriceController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController {
	...
	protected $bearbeitungen = array(
		array(
			...
			'functions' => array('berechneZuschnitt', 'berechneMacryl', 'berechneSchwabeln'),
		), array(
			'id' => 5,
	...
	public function getWerbelandRabatt() {
		return $this->werbelandRabatt;
	}
	...
	public function startCalculation() {
		//$this->debugTypo($this->cartItems, 'Preise vor Kalkulation', true);
	...
	public function getFaktoren() {
		...
		$partner = $this->partnerRepository->findByKunde($GLOBALS['TSFE']->fe_user->user['uid']);
		
		if(count($partner) > 0){
			if($this->werbelandRabatt = $partner->getFirst()->isBestaetigt()){
				$this->werbelandRabatt = $partner->getFirst()->getPartnerListe()->getRabatt();		
			}
		}
		
		$this->debugTypo($this->werbelandRabatt, 'Rabatt');
	...
	public function calculate($cartItem) {
		...
		$this->debugTypo($cartItem, 'CartItem: ');
	...
	public function berechnePacken($cartItem) {
		...
		if ($kilo > 40) {
			...
			$this->debugTypo(array('Lanege' => $laenge, 'Breite' => $breite, 'Dicke' => $dicke, 'Gurtmass' => $gurtmass), 'Masse');
		...
		if ($weight < 38) {
			...
			if (($this->shipping['Paketdienst']['gurtmass']['MAX'] > $aGurtmass) && (!$this->rahmenSpedition)) {
				if (($this->shipping['Paketdienst']['gurtmass']['L'] >= $laenge) && ($this->shipping['Paketdienst']['gurtmass']['B'] >= $breite)) { //&& ($this->shipping['Paketdienst']['gurtmass']['H'] >= $dicke)
					...
					//$this->debugTypo($pPreise, 'Versandkosten', true);
				...
				$this->debugTypo($shipping, 'Kosten', false);
			} else {
				$plz = null;
				if (isset($zip)) {
					$plz = substr($zip, 0, 2);
