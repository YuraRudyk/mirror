...
class AjController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController {
	...
	public function getAbschlussAction($params = array(), \TYPO3\CMS\Core\Http\AjaxRequestHandler &$ajaxObj = NULL) {
		...
		if ($args['generate']) {
			$excel = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('Excel');

			$excel->initialize($excel::ABSCHLUSS, array('fileName' => 'Abschluss_' . $monate[$monat] . '_' . $jahr . '.xls'));

			$res = $excel->write('Tabelle1', $abschlussData);
			$ajaxObj->addContent('glshop', json_encode($res));
		} else {
	...
	function sendMail($to, $subject, $message, $pfad) {
		...
		//$anhang enthÃƒÂ¤lt mehrere Dateien
		...
		if (@mail($to, $subject, $content, $header)) {
			...
			@mail('info@glacryl.de', $subject, $content, $header); // FÃƒÂ¼r Live Ãƒâ€žndern auf glacryl @ToDo
			return true;
		} else {
			return false;
		}
