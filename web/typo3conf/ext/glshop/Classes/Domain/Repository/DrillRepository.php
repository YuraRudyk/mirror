 ...
 ***/
/**
 * The repository for Drills
 */
class DrillRepository extends \TYPO3\CMS\Extbase\Persistence\Repository
{
    ...
    public function getBohrungen()
    {
        $array = [];
        $query = $this->createQuery();
        $bohrungen = $query->execute();
        for ($i = 0; $i < count($bohrungen); $i++) {
            $arr1 = [
    "uid" => $bohrungen[$i]->getUid(),
"pid" => $bohrungen[$i]->getPid(),
"name" => $bohrungen[$i]->getName(),
"bild" => $bohrungen[$i]->getPic(),
"preis" => $bohrungen[$i]->getPrice(),
"varianten" => []
];
            $varianten = $bohrungen[$i]->getDrilloption();
